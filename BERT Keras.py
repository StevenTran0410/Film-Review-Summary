...
from keras.layers import Layer, Embedding, Add, Dropout, Dense, LayerNormalization, Input, MultiHeadAttention
...
class BERTEmbeddingTF(Layer):
    ...
    def get_config(self):
        return super().get_config().update({"vocab_size": self.vocab_size, "embed_size": self.embed_size, "seq_len": self.seq_len})
...
def FeedForwardTF(input_shape, d_model, d_ff, dropout=0.1, prefix='ff'):
    ...
    x = Dense(d_ff, name = f'{prefix}_ff1', activation = 'gelu')(inputs)
    x = Dense(d_model, name = f'{prefix}_ff2', activation = 'gelu')(x)
...
def MultiHeadAttentionTF(input_shape, mask = 'False', prefix = "att", **kwargs):
    inputs = Input(input_shape, dtype=tf.float32, name=f'{prefix}_input')
    attention = MultiHeadAttention(name=f"{prefix}_attn1", **kwargs)
    x = attention(query=inputs, value=inputs, key=inputs, use_causal_mask=mask)
    x = Add(name = f'{prefix}_add')([inputs, x])
    x = LayerNormalization(name = f'{prefix}_ln')(x)
